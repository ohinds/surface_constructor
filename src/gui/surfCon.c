...
/* mode of operation */
enum MODE {BLANK, ALIGN, TACK, SEED};//, LACE};
int mode = BLANK;
...
void idle(void) {
  ...
  switch(mode) {
      ...
      case TACK:
        ...
        break;
      case SEED:
        if(seedRepaintNeeded(glutGet(GLUT_ELAPSED_TIME))) {
          redisplay();
        }
        break;
/*   case LACE: */
...
void createSurfMenu() {
  ...
  glutAddMenuEntry("'W' enter seed mode",'W');
...
void mouse(int button, int state, int x, int y) {
  ...
  switch(mode) {
      ...
      case TACK:
        ...
        break;
      case SEED:
        seedMouse(button, state, mousePos);
        break;
/*   case LACE: */
